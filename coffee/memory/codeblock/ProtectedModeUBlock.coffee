...
class ProtectedModeUBlock extends MicrocodeSet
	constructor: (@microcodes, x86lengths) ->
		...
		for i in [0...@parityMap.length]
			@parityMap[i] = (bitand(numberOfSetBits(i), 0x1) == 0)
		...
		try
			while @position < @microcodes.length
				...
				switch code
					...
					when @LOAD0_AX
						reg0 = bitand proc.eax, 0xffff
						mod = true

					when @LOAD0_CX
						reg0 = bitand proc.ecx, 0xffff
						mod = true

					when @LOAD0_DX
						reg0 = bitand proc.edx, 0xffff
						mod = true

					when @LOAD0_BX
						reg0 = bitand proc.ebx, 0xffff
						mod = true

					when @LOAD0_SP
						reg0 = bitand proc.esp, 0xffff
						mod = true

					when @LOAD0_BP
						reg0 = bitand proc.ebp, 0xffff
						mod = true

					when @LOAD0_SI
						reg0 = bitand proc.esi, 0xffff
						mod = true

					when @LOAD0_DI
						reg0 = bitand proc.edi, 0xffff
						mod = true

					when @STORE0_AX
						proc.eax = bitand(proc.eax, ~0xffff) | bitand(reg0, 0xffff)

					when @STORE0_CX
						proc.ecx = bitand(proc.ecx, ~0xffff) | bitand(reg0, 0xffff)

					when @STORE0_DX
						proc.edx = bitand(proc.edx, ~0xffff) | bitand(reg0, 0xffff)

					when @STORE0_BX
						proc.ebx = bitand(proc.ebx, ~0xffff) | bitand(reg0, 0xffff)

					when @STORE0_SP
						proc.esp = bitand(proc.esp, ~0xffff) | bitand(reg0, 0xffff)

					when @STORE0_BP
						proc.ebp = bitand(proc.ebp, ~0xffff) | bitand(reg0, 0xffff)

					when @STORE0_SI
						proc.esi = bitand(proc.esi, ~0xffff) | bitand(reg0, 0xffff)

					when @STORE0_DI
						proc.edi = bitand(proc.edi, ~0xffff) | bitand(reg0, 0xffff)

					when @STORE1_AX
						proc.eax = bitand(proc.eax, ~0xffff) | bitand(reg1, 0xffff)

					when @STORE1_CX
						proc.ecx = bitand(proc.ecx, ~0xffff) | bitand(reg1, 0xffff)

					when @STORE1_DX
						proc.edx = bitand(proc.edx, ~0xffff) | bitand(reg1, 0xffff)

					when @STORE1_BX
						proc.ebx = bitand(proc.ebx, ~0xffff) | bitand(reg1, 0xffff)

					when @STORE1_SP
						proc.esp = bitand(proc.esp, ~0xffff) | bitand(reg1, 0xffff)

					when @STORE1_BP
						proc.ebp = bitand(proc.ebp, ~0xffff) | bitand(reg1, 0xffff)

					when @STORE1_SI
						proc.esi = bitand(proc.esi, ~0xffff) | bitand(reg1, 0xffff)

					when @STORE1_DI
						proc.edi = bitand(proc.edi, ~0xffff) | bitand(reg1, 0xffff)

					when @LOAD1_AX
						reg1 = bitand proc.eax, 0xffff

					when @LOAD1_CX
						reg1 = bitand proc.ecx, 0xffff

					when @LOAD1_DX
						reg1 = bitand proc.edx, 0xffff

					when @LOAD1_BX
						reg1 = bitand proc.ebx, 0xffff

					when @LOAD1_SP
						reg1 = bitand proc.esp, 0xffff

					when @LOAD1_BP
						reg1 = bitand proc.ebp, 0xffff

					when @LOAD1_SI
						reg1 = bitand proc.esi, 0xffff

					when @LOAD1_DI
						reg1 = bitand proc.edi, 0xffff

					when @LOAD0_AL
						reg0 = bitand proc.eax, 0xff
						mod = true

					when @LOAD0_CL
						reg0 = bitand proc.ecx, 0xff
						mod = true

					when @LOAD0_DL
						reg0 = bitand proc.edx, 0xff
						mod = true

					when @LOAD0_BL
						reg0 = bitand proc.ebx, 0xff
						mod = true

					when @LOAD0_AH
						reg0 = bitand((proc.eax >> 8), 0xff)
						mod = true

					when @LOAD0_CH
						reg0 = bitand((proc.ecx >> 8), 0xff)
						mod = true

					when @LOAD0_DH
						reg0 = bitand((proc.edx >> 8), 0xff)
						mod = true

					when @LOAD0_BH
						reg0 = bitand((proc.ebx >> 8), 0xff)
						mod = true

					when @STORE0_AL
						proc.eax = bitand(proc.eax, ~0xff) | bitand(reg0, 0xff)

					when @STORE0_CL
						proc.ecx = bitand(proc.ecx, ~0xff) | bitand(reg0, 0xff)

					when @STORE0_DL
						proc.edx = bitand(proc.edx, ~0xff) | bitand(reg0, 0xff)

					when @STORE0_BL
						proc.ebx = bitand(proc.ebx, ~0xff) | bitand(reg0, 0xff)

					when @STORE0_AH
						proc.eax = bitand(proc.eax, ~0xff00) | bitand((reg0 << 8), 0xff00)

					when @STORE0_CH
						proc.ecx = bitand(proc.ecx, ~0xff00) | bitand((reg0 << 8), 0xff00)

					when @STORE0_DH
						proc.edx = bitand(proc.edx, ~0xff00) | bitand((reg0 << 8), 0xff00)

					when @STORE0_BH
						proc.ebx = bitand(proc.ebx, ~0xff00) | bitand((reg0 << 8), 0xff00)

					when @LOAD1_AL
						reg1 = bitand proc.eax, 0xff

					when @LOAD1_CL
						reg1 = bitand proc.ecx, 0xff

					when @LOAD1_DL
						reg1 = bitand proc.edx, 0xff

					when @LOAD1_BL
						reg1 = bitand proc.ebx, 0xff

					when @LOAD1_AH
						reg1 = bitand((proc.eax >> 8), 0xff)

					when @LOAD1_CH
						reg1 = bitand((proc.ecx >> 8), 0xff)

					when @LOAD1_DH
						reg1 = bitand((proc.edx >> 8), 0xff)

					when @LOAD1_BH
						reg1 = bitand((proc.ebx >> 8), 0xff)

					when @STORE1_AL
						proc.eax = bitand(proc.eax, ~0xff) | bitand(reg1, 0xff)

					when @STORE1_CL
						proc.ecx = bitand(proc.ecx, ~0xff) | bitand(reg1, 0xff)

					when @STORE1_DL
						proc.edx = bitand(proc.edx, ~0xff) | bitand(reg1, 0xff)

					when @STORE1_BL
						proc.ebx = bitand(proc.ebx, ~0xff) | bitand(reg1, 0xff)

					when @STORE1_AH
						proc.eax = bitand(proc.eax, ~0xff00) | bitand((reg1 << 8), 0xff00)

					when @STORE1_CH
						proc.ecx = bitand(proc.ecx, ~0xff00) | bitand((reg1 << 8), 0xff00)

					when @STORE1_DH
						proc.edx = bitand(proc.edx, ~0xff00) | bitand((reg1 << 8), 0xff00)

					when @STORE1_BH
						proc.ebx = bitand(proc.ebx, ~0xff00) | bitand((reg1 << 8), 0xff00)
					...
					when @LOAD0_ES
						reg0 = bitand 0xffff, proc.es.getSelector()
						mod = true

					when @LOAD0_CS
						reg0 = bitand 0xffff, proc.cs.getSelector()
						mod = true

					when @LOAD0_SS
						reg0 = bitand 0xffff, proc.ss.getSelector()
						mod = true

					when @LOAD0_DS
						reg0 = bitand 0xffff, proc.ds.getSelector()
						mod = true

					when @LOAD0_FS
						reg0 = bitand 0xffff, proc.fs.getSelector()
						mod = true

					when @LOAD0_GS
						reg0 = bitand 0xffff, proc.gs.getSelector()
						mod = true
					...
					when @STORE0_FLAGS
						proc.setEFlags(bitand(proc.getEFlags(), ~0xffff) | bitand(reg0, 0xffff))

					when @STORE0_EFLAGS
						proc.setEFlags(reg0)

					when @LOAD0_FLAGS
						reg0 = bitand 0xffff, proc.getEFlags()
						mod = true
					...
					when @LOAD0_IB
						reg0 = bitand @microcodes[@position++], 0xff
						mod = true

					when @LOAD0_IW
						reg0 = bitand @microcodes[@position++], 0xffff
						mod = true
					...
					when @LOAD1_IB
						reg1 = bitand @microcodes[@position++], 0xff

					when @LOAD1_IW
						reg1 = bitand @microcodes[@position++], 0xffff
					...
					when @LOAD2_AX
						reg2 = bitand proc.eax, 0xffff

					when @LOAD2_AL
						reg2 = bitand proc.eax, 0xff

					when @LOAD2_CL
						reg2 = bitand proc.ecx, 0xffff

					when @LOAD2_IB
						reg2 = bitand @microcodes[@position++], 0xff
					...
					when @LOAD0_MEM_BYTE
						if (!use)
							log "use not set."
						reg0 = bitand 0xff, seg0.getByte(addr0)
						mod = true

					when @LOAD0_MEM_WORD
						if (!use)
							log "use not set."
						reg0 = bitand 0xffff, seg0.getWord(addr0)
						mod = true
						# hiero.
					...
					when @LOAD1_MEM_BYTE
						if (!use)
							log "use not set."
						reg1 = bitand 0xff, seg0.getByte(addr0)

					when @LOAD1_MEM_WORD
						if (!use)
							log "use not set."
						reg1 = bitand 0xffff, seg0.getWord(addr0)
					...
					when @ROL_O8
						reg2 = bitand reg1, 0x7
						reg0 = (reg0 << reg2) | (reg0 >>> (8 - reg2))
						mod = true

					when @ROL_O16
						reg2 = bitand reg1, 0xf
					...
					when @RCL_O32
						reg1 &= 0x1f
						reg0l = bitand(0xffffffff, reg0) | (proc.getCarryFlag() ? 0x100000000 : 0)
					...
					when @RCR_O8
						...
						reg1 %= 9
						reg0 |= (proc.getCarryFlag() ? 0x100 : 0)
						reg2 = (proc.getCarryFlag() ^ (bitand(reg0, 0x80) != 0) ? 1:0)
						reg0 = (reg0 >>> reg1) | (reg0 << (9 - reg1))
						mod = true
					...
					when @RCR_O16
						reg1 &= 0x1f
						reg1 %= 17
						reg2 = (proc.getCarryFlag() ^ (bitand(reg0, 0x8000) != 0) ? 1:0)
						reg0 |= (proc.getCarryFlag() ? 0x10000 : 0)
						reg0 = (reg0 >>> reg1) | (reg0 << (17 - reg1))
						mod = true
					...
					when @RCR_O32
						reg1 &= 0x1f
						reg0 = bitand(0xffffffff, reg0) | (proc.getCarryFlag() ? 0x100000000 : 0)
						reg2 = (proc.getCarryFlag() ^ (bitand(reg0, 0x80000000) != 0) ? 1:0)
						reg0 = int(reg0l = (reg0l >>> reg1) | (reg0l << (33 - reg1)))
						mod = true
					...
					when @SHLD_O16
						...
						if (reg2 < 16)
							...
							i = bitand(reg1, 0xFFFF) | (reg0 << 16)
							reg0 = (reg1 << (reg2 - 16)) | (bitand(reg0, 0xFFFF) >>> (32 - reg2))
							reg1 = reg2 - 15
							reg2 = i >> 1
						mod = true
					...
					when @CWD
						if (bitand(proc.eax, 0x8000) == 0)
							proc.edx &= 0xffff0000
						else
							proc.edx |= 0x0000ffff

					when @CDQ
						if (bitand(proc.eax, 0x80000000) == 0)
							proc.edx = 0
						else
							proc.edx = -1
					...
					when @CLI
						if (proc.getIOPrivilegeLevel() >= proc.getCPL())
							...
							if ((proc.getIOPrivilegeLevel() < proc.getCPL()) && (proc.getCPL() == 3) && (bitand(proc.getCR4(), 1) != 0))
								proc.eflagsInterruptEnableSoon = false
							else
					...
					when @STI
						if (proc.getIOPrivilegeLevel() >= proc.getCPL())
							...
							if ((proc.getIOPrivilegeLevel() < proc.getCPL()) && (proc.getCPL() == 3) && (bitand(proc.getEFlags(), (1 << 20)) == 0))
								proc.eflagsInterruptEnableSoon = true
							else
								throw new ProcessorException(Type.GENERAL_PROTECTION, 0, true)
...
#						@jump_o8(byte(reg0))
					...
					when @SMSW
						reg0 = bitand proc.getCR0(), 0xffff
						mod = true

					when @LMSW
						if (proc.getCPL() != 0)
							throw new ProcessorException(Type.GENERAL_PROTECTION, 0, true)
						proc.setCR0(bitand(proc.getCR0(), ~0xe) | bitand(reg0, 0xe))
					...
					when @CMPXCHG8B
						edxeax = (bitand(proc.edx, 0xffffffff) << 32) | bitand(proc.eax, 0xffffffff)
						if (edxeax == reg0)
							proc.setZeroFlag(true)
							reg0 = (bitand(proc.ecx, 0xffffffff) << 32) | bitand(proc.ebx, 0xffffffff)
						else
					...
					when @PUSH_O32_A16, @PUSH_O32_A32
						if (proc.ss.getDefaultSizeFlag())
							...
							@push_o32_a16(reg0) # Stond nog een byte(reg0) voor?
					...
					when @PUSHF_O32_A16, @PUSHF_O32_A32
						if (proc.ss.getDefaultSizeFlag())
							@push_o32_a32(bitand ~0x30000, reg0)
						else
							@push_o32_a16(bitand ~0x30000, reg0)
					...
					when @POP_O32_A16, @POP_O32_A32
						if (proc.ss.getDefaultSizeFlag())
							...
							reg1 = bitand(proc.esp, ~0xffff) | bitand((proc.esp + 4), 0xffff)
						if (@microcodes[@position] == @STORE0_SS)
							proc.eflagsInterruptEnable = false
						reg0 = proc.ss.getDoubleWord(bitand 0xffff, proc.esp)
						mod = true
					...
					when @POP_O16_A16, @POP_O16_A32
						if (proc.ss.getDefaultSizeFlag())
							...
							if (@microcodes[@position] == @STORE0_SS)
								proc.eflagsInterruptEnable = false
							reg0 = bitand 0xffff, proc.ss.getWord(proc.esp)
						else
							reg1 = bitand(proc.esp, ~0xffff) | bitand((proc.esp + 2), 0xffff)
							if (@microcodes[@position] == @STORE0_SS)
								proc.eflagsInterruptEnable = false
							reg0 = bitand 0xffff, proc.ss.getWord(bitand 0xffff, proc.esp)
						mod = true
					...
					when @POPF_O32_A16, @POPF_O32_A32
						if (proc.ss.getDefaultSizeFlag())
							...
							reg0 = proc.ss.getDoubleWord(bitand 0xffff, proc.esp)
							proc.esp = bitand(proc.esp, ~0xffff) | bitand((proc.esp + 4), 0xffff)
						if (proc.getCPL() == 0)
							reg0 = (bitand(proc.getEFlags(), 0x20000) | bitand(reg0, ~(0x20000 | 0x180000)))
						else
							if (proc.getCPL() > proc.eflagsIOPrivilegeLevel)
								reg0 = (bitand(proc.getEFlags(), 0x23200) | bitand(reg0, ~(0x23200 | 0x180000)))
							else
								reg0 = (bitand(proc.getEFlags(), 0x23000) | bitand(reg0, ~(0x23000 | 0x180000)))
						mod = true
					...
					when @POPF_O16_A16, @POPF_O16_A32
						if (proc.ss.getDefaultSizeFlag())
							...
							reg0 = bitand 0xffff, proc.ss.getWord(bitand 0xffff, proc.esp)
							proc.esp = bitand(proc.esp, ~0xffff) | bitand((proc.esp + 2), 0xffff)
						if (proc.getCPL() != 0)
							if (proc.getCPL() > proc.eflagsIOPrivilegeLevel)
								reg0 = (bitand(proc.getEFlags(), 0x3200) | bitand(reg0, ~0x3200))
							else
								reg0 = (bitand(proc.getEFlags(), 0x3000) | bitand(reg0, ~0x3000))
						mod = true
					...
					when @LGDT_O16
						proc.gdtr = proc.createDescriptorTableSegment(bitand(reg1, 0x00ffffff), reg0)

					when @LGDT_O32
						proc.gdtr = proc.createDescriptorTableSegment(reg1, reg0)

					when @SGDT_O16
						reg1 = bitand proc.gdtr.getBase(), 0x00ffffff
					...
					when @LIDT_O16
						proc.idtr = proc.createDescriptorTableSegment(bitand(reg1, 0x00ffffff), reg0)

					when @LIDT_O32
						proc.idtr = proc.createDescriptorTableSegment(reg1, reg0)

					when @SIDT_O16
						reg1 = bitand proc.idtr.getBase(), 0x00ffffff
					...
					when @SLDT
						reg0 = bitand 0xffff, proc.ldtr.getSelector()
						mod = true
					...
					when @STR
						reg0 = bitand 0xffff, proc.tss.getSelector()
						mod = true

					when @VERR
						try
							test = proc.getSegment(bitand reg0, 0xffff)
							type = test.getType()
							if ((bitand(type, ProtectedModeSegment.DESCRIPTOR_TYPE_CODE_DATA) == 0) || ((bitand(type, ProtectedModeSegment.TYPE_CODE_CONFORMING) == 0) && ((proc.getCPL() > test.getDPL()) || (test.getRPL() > test.getDPL()))))
								proc.setZeroFlag(false)
							else
								proc.setZeroFlag((bitand(type, ProtectedModeSegment.TYPE_CODE) == 0) || (bitand(type, ProtectedModeSegment.TYPE_CODE_READABLE) != 0))
						catch e
							proc.setZeroFlag(false)
					...
					when @VERW
						try
							...
							if ((bitand(type, ProtectedModeSegment.DESCRIPTOR_TYPE_CODE_DATA) == 0) || ((bitand(type, ProtectedModeSegment.TYPE_CODE_CONFORMING) == 0) && ((proc.getCPL() > test.getDPL()) || (test.getRPL() > test.getDPL()))))
								...
								proc.setZeroFlag((bitand(type, ProtectedModeSegment.TYPE_CODE) == 0) && (bitand(type, ProtectedModeSegment.TYPE_DATA_WRITABLE) != 0))
						catch e
							proc.setZeroFlag(false)
					...
					when @CLTS
						if (proc.getCPL() != 0)
							throw new ProcessorException(Type.GENERAL_PROTECTION, 0, true)
						proc.setCR3(bitand proc.getCR3(), ~0x4)
					...
					when @WRMSR
						if (proc.getCPL() != 0)
							throw new ProcessorException(Type.GENERAL_PROTECTION, 0, true)
						proc.setMSR(reg0, (bitand reg2, 0xffffffff) | ((bitand reg1, 0xffffffff) << 32))
					...
					when @CMPXCHG_O32_FLAGS
						@sub_o32_flags((bitand 0xffffffff, reg2) - (bitand 0xffffffff, reg1), reg2, reg1)
	...
	cmpsb_a32: (seg0) ->
		addrOne = proc.esi
		addrTwo = proc.edi
		int dataOne = bitand 0xff, seg0.getByte(addrOne)
		int dataTwo = bitand 0xff, proc.es.getByte(addrTwo)
	...
	cmpsw_a32: (seg0) ->
		addrOne = proc.esi
		addrTwo = proc.edi
		dataOne = bitand 0xffff, seg0.getWord(addrOne)
		dataTwo = bitand 0xffff, proc.es.getWord(addrTwo)
		if (proc.eflagsDirection)
			addrOne -= 2
			addrTwo -= 2
		else
	...
	cmpsd_a32: (seg0) ->
		...
		proc.esi = addrOne
		proc.edi = addrTwo
		sub_o32_flags(bitand(0xffffffff, dataOne) - bitand(0xffffffff, dataTwo), dataOne, dataTwo)

	repe_cmpsb_a16: (seg0) ->
		count = bitand 0xFFFF, proc.ecx
		addrOne = bitand 0xFFFF, proc.esi
		addrTwo = bitand 0xFFFF, proc.edi
		used = count != 0
		dataOne = 0
		dataTwo = 0
		try
			if (proc.eflagsDirection)
				while (count != 0)
					...
					dataOne = bitand 0xff, seg0.getByte(addrOne)
					dataTwo = bitand 0xff, proc.es.getByte(addrTwo)
					count--
					addrOne -= 1
					addrTwo -= 1
					if (dataOne != dataTwo)
			...
			@executeCount += (bitand(proc.ecx, 0xFFFF)  - count)
			proc.ecx = bitand(proc.ecx, ~0xFFFF) | bitand(count, 0xFFFF)
			proc.esi = bitand(proc.esi, ~0xFFFF) | bitand(addrOne, 0xFFFF)
			proc.edi = bitand(proc.edi, ~0xFFFF) | bitand(addrTwo, 0xFFFF)
			if (used)
				sub_o8_flags(dataOne - dataTwo, dataOne, dataTwo)
	repe_cmpsb_a32: (seg0) ->
		count = proc.ecx
	...
	lodsb_a16: (dataSegment) ->
		addr = bitand 0xFFFF, proc.esi ;
		proc.eax = bitand(proc.eax, ~0xff) | bitand(0xff, dataSegment.getByte(addr))
		if (proc.eflagsDirection)
			addr -= 1
		else
			addr += 1
		proc.esi = bitand(proc.esi, ~0xffff) | bitand(0xffff,  addr);
	...
	lodsw_a16: (dataSegment) ->
		int addr = bitand proc.esi, 0xFFFF;
		proc.eax = bitand(proc.eax, ~0xffff) | bitand(0xffff, dataSegment.getWord(addr))
		if (proc.eflagsDirection)
			addr -= 2
		else
			addr += 2
	...
	rep_lodsb_a32: (dataSegment) ->
		count = proc.ecx
		addr = proc.esi
		data = bitand proc.eax, 0xff
		@executeCount += count
		try
			if (proc.eflagsDirection)
				while (count != 0)
					#check hardware interrupts
					data = bitand 0xff, dataSegment.getByte(addr)
					count--
					addr -= 1
			else
				while (count != 0)
			...
			proc.eax = (bitand proc.eax, ~0xff) | data
			proc.ecx = count
			proc.esi = addr
	rep_lodsw_a32: (dataSegment) ->
		count = proc.ecx
		addr = proc.esi
		data = bitand proc.eax, 0xffff
		@executeCount += count
		try
			if (proc.eflagsDirection)
				while (count != 0)
					#check hardware interrupts
					data = bitand 0xffff, dataSegment.getWord(addr)
					count--
					addr -= 2
			else
				while (count != 0)
			...
			proc.eax = (bitand proc.eax, ~0xffff) | data
			proc.ecx = count
			proc.esi = addr
	...
	movsb_a16: (outSegment) ->
		inAddr = bitand proc.edi, 0xffff
		outAddr = bitand proc.esi, 0xffff
		...
		if (proc.eflagsDirection)
			...
			outAddr += 1
			inAddr += 1
		proc.edi = bitand(proc.edi, ~0xffff) | bitand(inAddr, 0xffff)
		proc.esi = bitand(proc.esi, ~0xffff) | bitand(outAddr, 0xffff)
	...
	rep_movsb_a16: (outSegment) ->
		...
		try
			if (proc.eflagsDirection)
				while (count != 0)
					#check hardware interrupts
					proc.es.setByte(bitand(inAddr, 0xffff), outSegment.getByte(bitand outAddr, 0xffff))
					count--
					outAddr -= 1
					inAddr -= 1
			else
	...
	rep_movsd_a16: (outSegment) ->
		...
		try
			if (proc.eflagsDirection)
				while (count != 0)
					#check hardware interrupts
					proc.es.setDoubleWord(bitand(inAddr, 0xffff), outSegment.getDoubleWord(bitand outAddr, 0xffff))
					count--
					outAddr -= 4
					inAddr -= 4
			else
...
#		a.a()
	...
	# TODO:  supposed to throw SS exception
	# "if top 6 bytes of stack not within stack limits"
		log "Returned value: #{proc.ss.getDoubleWord(proc.esp)}, esp: #{proc.esp}"
		#
